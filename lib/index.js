...
function resolveBaseURL(baseURL, allowCustom) {
  const canonical = (baseURL ?? TAVILY_DEFAULT_BASE_URL).replace(/\/+$/u, "");
  if (canonical === TAVILY_DEFAULT_BASE_URL) return canonical;
  if (allowCustom !== true) {
    throw new WebError(
      "custom baseURL " + JSON.stringify(canonical) + " is not allowed; only the official " + TAVILY_DEFAULT_BASE_URL + " is permitted unless allowCustomBaseURL: true",
      "WEB_PROVIDER_ERROR"
    );
  }
  ...
  async search(request, signal) {
    ...
    try {
      response = await fetch(endpoint, {
        ...
        ...(withTimeout(signal, options.searchTimeoutMs) !== void 0 ? { signal: withTimeout(signal, options.searchTimeoutMs) } : {})
      });
    } catch (error) {
      if (error instanceof DOMException && error.name === "TimeoutError") {
        throw new WebError("Tavily search timed out after " + options.searchTimeoutMs + "ms", "WEB_PROVIDER_ERROR", { cause: error });
      }
...
export { TAVILY_PROVIDER_ID, TAVILY_DEFAULT_BASE_URL, DEFAULT_API_KEY_ENV, TavilySearchProvider, mapTavilyResponse, resolveBaseURL, withTimeout };
